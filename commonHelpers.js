import{g as p,i as q,a as W,b as j,c,d as v,e as h,f as y,s as w,r as K,h as L,j as C}from"./assets/scroll-up-b81b2e65.js";import{b as O}from"./assets/vendor-60358816.js";const{bookIdsLSKey:k}=K,d={};async function z(o){const t=o.target.closest("li");if(t)try{let m=function(i){i.key==="Escape"&&g.close()};const e=t.dataset.bookId,r=(await p(e)).data,{book_image:n,title:a,author:l,description:x,buy_links:A}=r,[M,_]=A,I=`<div class="modal-window">
  ...
  </div>`,g=O.create(I,{onShow:i=>{i.element().querySelector(".modal-window-close-btn").onclick=i.close,window.addEventListener("keydown",m),document.body.style.overflow="hidden"},onClose:()=>{window.removeEventListener("keydown",m),document.body.style.overflow="";const{shopListBtn:i}=d;i.removeEventListener("click",S)}});g.show(),document.querySelector(".basicLightbox").style.backgroundColor="rgba(17, 17, 17, 0.4)",document.querySelector(".basicLightbox").style.overflowY="auto",D(e)}catch(e){console.error(e),c(!1,"Server Error")}}function D(o){d.shopListBtn=document.querySelector(".modal-window-book-shopping-list-button"),d.congratulationsText=document.querySelector(".modal-window-congratulations");const{shopListBtn:t}=d;b(o),t.addEventListener("click",()=>{S(o)})}function S(o){const t=b(o),e=v(k)||[];if(t){const s=e.indexOf(o);e.splice(s,1),h(k,e),y&&w(e),b(o)}else e.push(o),h(k,e),y&&w(e),b(o)}function b(o){const{congratulationsText:t,shopListBtn:e}=d,r=(v(k)||[]).includes(o);return r?(e.textContent="remove from the shopping list",t.classList.toggle("visually-hidden")):e.textContent="add to shopping list",r}const u=document.querySelector(".books-container");async function H(){try{return(await p("top-books")).data}catch(o){console.error(o)}}async function $(){try{L();const t=(await H()).map(r=>{const n=r.books,a=r.list_name;return N(n,a)}).join(""),s=E("Best Sellers Books")+t;u.innerHTML=s,T(),C()}catch(o){console.error(o),c(!1,"Server Error")}}function N(o,t){const e=o.map(r=>R(r)).join("");return`<div class="category-books-wrapper"><h2 class="category-title-bestsellers">${t}</h2><ul class="books-list">${e}</ul><button type="button" class="books-list-btn"" data-category-name="${t}">See more</button></div>`}function R({book_image:o,title:t,author:e,_id:s}){return`
